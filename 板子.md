...
ta=$1
src="${ta}.cpp"
g++ -std=gnu++20 -O2 -Wall -Wextra "$src" -o "$ta" && \
./"$ta" 2>&1 | tee "${ta}.txt"
```
...
//tree<key,mapped,cmp,tag,update_policy>
typedef tree<int,null_type,less<int>,rb_tree_tag,tree_order_statistics_node_update> ordered_set;
ordered_set s;
s.insert(x); //插入
s.find_by_order(k); //返回迭代器，指向从小到大排第k的元素 (0-based)
s.order_of_key(x); //返回x在集合中的排名（严格小于的元素个数）
```
...
卡特兰数的一般项公式为 $$C_n = \frac{1}{n+1} \binom{2n}{n} = \frac{(2n)!}{(n+1)!n!} = \binom{2n}{n} - \binom{2n}{n+1}$$  

递推式是 $$f(n) = \sum_{i=0}^{n-1} f(i) \cdot f(n-1-i)$$

常见的组合结构：

1. 表示长度为2n的dyck word个数，或者说n组括号的合法运算式的个数，或者说进出栈的置换个数。
2. 表示有n个节点组成不同构二叉树的方案数。
3. 表示有2n+1个节点组成不同构满二叉树的方案数。
4. 表示n*n格点中不越过对角线的单调路径的个数，此外，不是对角线的话，也是类似卡特兰数的结构，只需将(0,0)对称一下，合法路径=总路径-违规路径。
5. 表示通过连结顶点而将*n* + 2边的凸多边形分成三角形的方法个数。
...
int work(int x) {
	...
	auto dfs=[&](auto&& self,int pos,int sum,bool limit) -> int {
		if (pos == -1) return sum;
		...
		for(int i=0;i<=up;i++) {
			(res+=self(self,pos-1,sum+i,limit and (i == up)))%=p;
		}
		if (limit == 0) dp[pos][sum]=res;
		return res;
	};
	return dfs(dfs,a.size()-1,0,1);
}
```
...
在字符串理论中，既是前缀又是后缀的子串称为Border，next数组求的是每个前缀的最长非空Border（不包括前缀自己）。

如果想求最短非空Border，有一个性质是，如果s有最长Border s1，那么s的最短Border一定是s1的最短Border。
